        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-8">
              ...
              <div className="p-8">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    ...
                    className={`${
                      isEditing ? "bg-emerald-500 text-white" : "bg-indigo-50 text-indigo-600"
                  ...
                  <div className="flex flex-col items-center gap-3">
                    <div
                      ...
                      className={`relative w-28 h-28 rounded-full overflow-hidden border-4 ${
                        isEditing ? "border-emerald-400 ring-4 ring-emerald-500/10 cursor-pointer" : "border-slate-100"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 text-sm font-medium text-slate-800">
                          <span className="w-1.5 h-4 bg-indigo-600 rounded-full" /> Full Name
                        </label>
                        <div className="flex items-center justify-between bg-slate-100 px-4 py-3 rounded-2xl border border-slate-200">
                          <span className="text-slate-600">{studentInfo.fullName}</span>
                          <Lock size={14} className="text-slate-400" />
                        </div>
                      </div>
                      <div className="space-y-2">
                        <label className="flex items-center gap-2 text-sm font-medium text-slate-800">
                          <span className="w-1.5 h-4 bg-rose-400 rounded-full" /> Display Name
                        </label>
                        <input
                          type="text"
                          value={displayName}
                        ...
                        />
                        {isEditing && (
                          <p className="text-[10px] text-indigo-500 font-medium ml-2">กำลังอยู่ในโหมดแก้ไข...</p>
                        )}
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 text-sm font-medium text-slate-800">
                        <span className="w-1.5 h-4 bg-indigo-600 rounded-full" /> Email
                      </label>
                      <div className="flex items-center justify-between bg-slate-100 px-4 py-3 rounded-2xl border border-slate-200">
            ...
            <div className="bg-white rounded-[40px] p-10 shadow-sm border border-slate-100">
              <div className="flex items-center gap-4 mb-8">
                ...
                <h2 className="text-xl font-bold text-slate-800">การตั้งค่าการแจ้งเตือน</h2>
              </div>
              <div className="space-y-8">
                {[
                  { id: "answers", title: "การตอบคำถาม", desc: "ได้รับแจ้งเตือนเมื่ออาจารย์หรือนักศึกษาตอบคำถาม" },
                  { id: "liveBoard", title: "Live Board", desc: "ได้รับแจ้งเตือนเมื่อมี Live Board" },
                  { id: "gmail", title: "Gmail Notification", desc: "รับแจ้งเตือนผ่าน Email" },
                ].map((item) => (
                  <div key={item.id} className="flex justify-between items-center">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      ...
                      />
                      <div className="w-14 h-8 bg-slate-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:start-[4px] after:bg-white after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-indigo-500" />
                    </label>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-[#F6F3F2] rounded-[40px] p-8 shadow-sm border border-slate-100">
            <div className="flex items-center gap-2 mb-8 text-rose-500">
              <Activity size={22} />
              <h2 className="text-xl font-medium text-slate-800">Academic Pulse</h2>
            </div>
            <div className="space-y-4">
              <div className="bg-slate-50 p-5 rounded-3xl flex justify-between items-center border border-slate-100">
                ...
                <span className="text-lg font-medium text-indigo-600">{studentInfo.studentId}</span>
              </div>
              <div className="bg-slate-50 p-5 rounded-3xl flex justify-between items-center border border-slate-100">
                <span className="text-sm font-medium text-slate-500">Semester</span>
                <span className="text-lg font-medium text-slate-800">{studentInfo.semester}</span>
              </div>
              <div className="bg-slate-50 p-5 rounded-3xl flex justify-between items-center border border-slate-100">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-slate-500">Enrolled</span>
                  <span className="bg-rose-100 text-rose-500 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase">Active</span>
                </div>
                <span className="text-lg font-medium text-rose-500">{studentInfo.enrolledCourses} COURSES</span>
              </div>
            </div>
          </div>
        </div>
