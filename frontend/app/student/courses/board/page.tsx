...
import type { AskQuestionPayload } from "../../../components/askmodal";
...
const LabDiscussionPage = () => {
    ...
    const handleAddDiscussion = (payload: AskQuestionPayload) => {
        const newCard = {
            id: Date.now(), 
    ...
    const [discussions, setDiscussions] = useState([
        ...
        },
        {
            ...
            status: "IN DISCUSSION",
            createdAt: new Date(Date.now() - 60000).toISOString(),
            content: "ทำไมต้องตั้ง Source เป็น Web Security Group แทนที่จะใส่ IP หรือ 0.0.0.0/0 แล้วมันส่งผลกับการเข้าถึง DB ยังไง?",
            user: "สมปอง กุ๊กกิ๊ก",
            avatar: "https://api.dicebear.com/7.x/bottts/svg?seed=1",
            isSolved: false
        },
        {
            ...
            status: "IN DISCUSSION",
            createdAt: new Date(Date.now() - 60000).toISOString(),
            content: "ทำไม RDS ต้องใช้ Subnet อย่างน้อย 2 Availability Zones และถ้าเลือกผิด subnet จะเกิดอะไรขึ้น?",
            user: "นิลมังกร",
        ...
        },
        {
            ...
            status: "ANSWERED",
            createdAt: new Date(Date.now() - 60000).toISOString(),
            content: "Multi-AZ ที่เปิดไว้ใน lab นี้ มีผลแค่ High Availability หรือมีผลกับ performance / cost ยังไง?",
            user: "ทวิตตี้",
        ...
        },
        {
            ...
            status: "ANSWERED",
            createdAt: new Date(Date.now() - 60000).toISOString(),
            content: "Endpoint ที่ได้ (เช่น lab-db.xxxx.rds.amazonaws.com) ต่างจาก IP ยังไง และทำไมต้องใช้ endpoint แทน IP?",
            user: "สิงหามาครับ",
        ...
        },
        {
            ...
            status: "ANSWERED",
            createdAt: new Date(Date.now() - 60000).toISOString(),
            content: "ถ้าใส่ endpoint + user + password ถูกแล้วแต่ connect ไม่ได้ ควรเช็คอะไรเป็นลำดับแรก?",
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFE] p-6 md:p-10 font-sans">
            <div className="max-w-7xl mx-auto">
                ...
                <header className="flex justify-between items-center mb-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-[#4A4A4A]">{labInfo.title}</h1>
                            <p className="text-slate-400 text-sm font-medium">Date : {labInfo.date}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center -space-x-3">
                        {labInfo.members.map((url, i) => (
                            <img key={i} src={url} className="w-10 h-10 rounded-full border-2 border-white shadow-sm" alt="member" />
                        ))}
            ...
            <AskModal isOpen={open} onClose={() => setOpen(false)} onAdd={handleAddDiscussion} />
        </div>
    );
};
...
function DiscussionCard({
    ...
    return (
        <div className={`relative flex flex-col justify-between p-7 rounded-[40px] h-[280px] shadow-sm border transition-all hover:shadow-md ${isInDiscussion
            ...
            <div>
                ...
                <div className="flex justify-between items-center mb-5">
                    <span className={`px-4 py-1 rounded-full text-[10px] font-bold tracking-wider ${isInDiscussion
                ...
                {/* Content */}
                <p className={`text-sm leading-relaxed font-medium line-clamp-5 ${isInDiscussion ? "text-[#5A5A5A]" : "text-[#5A8A6A]"
                    }`}>
                    {data.content}
                </p>
            </div>
            ...
            <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/50">
                <div className="flex items-center gap-2">
                    <img src={data.avatar} className="w-7 h-7 rounded-full bg-white shadow-sm" alt="user" />
