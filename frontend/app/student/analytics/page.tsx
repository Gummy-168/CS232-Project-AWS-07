  ...
  return (
    <div className="min-h-screen bg-slate-50 p-8 font-sans text-slate-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <StatCard label="PARTICIPATION RATE" value={`${data.stats.participation}%`} subValue="+4%" />
        <StatCard label="TOTAL QUESTIONS" value={data.stats.questions} isActive />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          />
                  {data.chart.map((entry: any, index: number) => (
                    <Cell
                      ...
                      fill={index === activeIndex ? 'url(#colorGradient)' : '#E2E8F0'}
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
          <h3 className="text-xl font-bold self-start mb-4">Question Status</h3>
          <div className="relative h-48 w-48">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-3xl font-bold">{data.stats.unanswered + data.stats.answered + data.stats.board}</span>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between items-center text-sm mb-1">
            ...
            <span className="text-indigo-600 font-semibold text-xs">1 interactions</span>
          </div>
...
const StatCard = ({ label, value, subValue }: any) => {
  ...
  const defaultLabelClasses = "text-[10px] font-bold text-slate-400 transition-colors duration-300 tracking-wider uppercase group-hover:text-pink-500";
  const defaultBorderClasses = "border-slate-100 transition-all duration-300";
  const hoverBorderClasses = "group-hover:border-pink-400 group-hover:shadow-lg group-hover:scale-105";
