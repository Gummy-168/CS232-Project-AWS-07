...
export default function BoardPage() {
  ...
  const handlePostReply = (id: number, reply: string) =>
    setQuestions((prev) =>
      prev.map((q) =>
        q.id === id
          ? { ...q, professorReplies: [...(q.professorReplies || []), reply] }
          : q,
      ),
    );
    ...
    setQuestions((prev) =>
      prev.map((q) =>
        q.id === id
          ? {
              ...q,
              professorReplies: q.professorReplies.filter(
...
function ActivityCard({
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-50 relative">
      <div
        className={`absolute top-5 right-5 flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold ${isAnswered ? "bg-emerald-50 text-emerald-500" : "bg-orange-50 text-orange-400"}`}
      >
        {isAnswered ? (
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1">
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="text-rose-400 hover:text-rose-600 text-xs font-bold px-1"
                  >
                    ✕
                  </button>
                </div>
          ...
          <div className="flex items-center gap-4 text-[13px]">
            <span className="text-slate-400">↩ {data.replies || 0}</span>
