...
export default function ProfessorCourseDashboardView() {
  ...
  const [joinCode, setJoinCode] = useState<ProfessorJoinCodeResponse | null>(
    null,
  );
  ...
  const [boardToClose, setBoardToClose] =
    useState<ProfessorBoardSession | null>(null);
  ...
  const [isDeleteSectionModalOpen, setIsDeleteSectionModalOpen] =
    useState(false);
  const [deleteSectionConfirmation, setDeleteSectionConfirmation] =
    useState("");
      ...
      .catch((err: unknown) => {
        const message =
          err instanceof Error
            ? err.message
            : "Failed to load course dashboard";
        setError(message);
      });
  }, [
    isHydrated,
  ...
  const activeBoard =
    sortedBoards.find(
      (board) => board.status.trim().toUpperCase() === "ACTIVE",
    ) ?? null;
  ...
  const boardCardTitle = activeBoard
    ...
    : "No active board";
  const boardCardSubtitle =
    !activeBoard && latestBoard
  ...
  const answeredCount = useMemo(
    () =>
      filteredQuestions.filter((question) => question.status === "ANSWERED")
        .length,
    [filteredQuestions],
  );
  ...
  const handleGenerateJoinCode = async () => {
    ...
    setJoinCodeError("");
    try {
      const generated = await createProfessorJoinCode(
        session.userId,
        selectedCourseCode,
        {
  ...
  const handleConfirmDeleteSection = async () => {
    ...
    if (
      deleteSectionConfirmation.trim().toUpperCase() !== expectedSectionLabel
    ) {
      setDeleteSectionError(
        `Please type ${expectedSectionLabel} exactly to continue.`,
      );
      return;
    }
    ...
    try {
      ...
      setDeleteSectionConfirmation("");
      router.replace(
        `${pathname}?course_code=${encodeURIComponent(selectedCourseCode)}`,
      );
      refresh();
      setActionNotice({
        tone: result.action === "deleted" ? "success" : "warning",
        badge:
          result.action === "deleted" ? "Section Deleted" : "Section Archived",
        title:
  ...
  return (
    <div className="min-h-screen bg-[#FCF9F8] text-slate-700">
      ...
      <ConfirmDeleteSectionModal
        ...
        sectionLabel={
          selectedSectionCode
            ? formatSectionLabel(selectedSectionCode)
            : "Section"
        }
        enrolledCount={
          data?.enrolled_students.filter(
            (student) =>
              student.section_code?.trim().toUpperCase() ===
              selectedSectionCode,
          ).length ?? 0
        }
        boardCount={
          data?.board_sessions.filter(
            (board) =>
              board.section_code?.trim().toUpperCase() === selectedSectionCode,
          ).length ?? 0
        }
        ...
        <div className="mx-auto w-full max-w-[1360px] px-6 xl:px-8 2xl:px-10">
          <section className="mb-8 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-dashed border-[#D8CFFE] bg-[#FFFBFF] px-4 py-2 text-sm font-medium text-[#6C56EC] transition hover:bg-white"
            >
              <Plus size={16} />
              Create Sec
            </button>
            {selectedSectionCode ? (
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-600 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <Trash2 size={16} />
                Delete Sec
              </button>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full border border-[#E6DFFD] bg-white px-4 py-2 text-sm font-medium text-[#6C56EC] transition hover:border-[#CBBEFF] hover:bg-[#FBF9FF]"
            >
              <Eye size={16} />
              Generate Sec Code
            </button>
          ...
          {isLoading ? (
            ...
            <div className="rounded-[28px] border border-white/70 bg-white px-8 py-16 text-center shadow-sm">
              ...
              <button
                ...
                className="mt-6 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#5B41FF] to-[#D94FA2] px-5 py-3 text-sm font-medium text-white shadow-lg shadow-violet-100"
              >
                <Plus size={16} />
                Create Course
              </button>
            ...
            <>
                      ...
                      <div className="w-full max-w-[360px] rounded-[28px] bg-[#FBF9FF] p-5">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3">
                          <div className="rounded-2xl bg-[#EEF0FF] px-4 py-3">
                            ...
                            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#7E73B6]">
                              Pending
                            </p>
                          </div>
                          <div className="rounded-2xl bg-[#FFF3E8] px-4 py-3">
                            ...
                            <p className="text-xs font-medium uppercase tracking-[0.12em] text-[#AA8A63]">
                              Questions
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          {activeBoard ? (
                            <Link
                              ...
                              className="flex-1 rounded-2xl bg-gradient-to-r from-[#5B41FF] to-[#D94FA2] px-4 py-3 text-center text-sm font-medium text-white shadow-lg shadow-violet-100"
                            >
                              Review Session
                            </Link>
                          ) : (
                            <button
                              ...
                              className="flex-1 rounded-2xl bg-gradient-to-r from-[#5B41FF] to-[#D94FA2] px-4 py-3 text-sm font-medium text-white shadow-lg shadow-violet-100"
                            >
                              Open Board
                            </button>
                          )}
                          {!activeBoard && reviewBoard ? (
                            <Link
                              ...
                              className="inline-flex items-center justify-center rounded-2xl border border-[#E1DAFA] px-4 py-3 text-sm font-medium text-[#6B5AD8] transition hover:bg-[#FBF9FF]"
                            >
                              Review Session
                            </Link>
                          ) : null}
                          {activeBoard ? (
                            <button
                              type="button"
                              onClick={() =>
                                handleRequestCloseBoard(activeBoard)
                              }
                              className="inline-flex items-center justify-center rounded-2xl border border-rose-200 px-4 py-3 text-sm font-medium text-rose-600 transition hover:bg-rose-50"
                            >
                              Close Board
                            </button>
                          ) : null}
                  ...
                  <div className="grid gap-3 grid-cols-2 xl:grid-cols-4">
                    {[
                      ...
                      },
                      {
                        label: "PENDING",
                        value: pendingCount,
                        icon: <Clock3 size={18} className="text-pink-500" />,
                      ...
                      <div
                        key={stat.label}
                        className="rounded-2xl bg-white px-5 py-4 shadow-sm flex items-center justify-between gap-3"
                      >
                        <div>
                          ...
                          <p
                            className={`mt-1 text-2xl font-bold ${stat.valueColor}`}
                          >
                            {stat.value}
                          </p>
                        </div>
                ...
                <aside className="rounded-[32px] bg-white p-5 xl:p-6 shadow-[0_18px_40px_rgba(104,74,191,0.08)]">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <Users size={17} className="text-[#6C56EC]" />
                    ...
                    <span className="rounded-full bg-[#F5F1FF] px-3 py-1 text-xs font-medium text-[#6C56EC]">
                      {studentSummaries.length} students
                    </span>
                  </div>

                  <div className="mt-5 flex items-center gap-3 bg-white border border-slate-200 py-2 pl-5 pr-0 rounded-full text-sm focus:outline-none">
                        ...
                        >
                          <div className="flex min-w-0 items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="truncate text-sm font-medium text-[#2A2340]">
                                {student.name}
                              </p>
                              <p className="mt-1 text-xs text-[#9B91C7]">
              ...
              <section className="mt-12">
                <div>
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-3 mb-4">
                    <div className="flex items-center gap-2 flex-wrap">
                      {(
                        ["all", "answered", "unanswered", "board"] as const
                      ).map((value) => (
                        <button
                          key={value}
                          type="button"
                            ...
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                              <div className="flex gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="mt-2 text-sm text-[#7E75A4]">
                                    ...
                                    {board.answered_questions} answered
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-wrap items-center gap-3">
                                ...
                                <Link
                                  ...
                                  className="inline-flex gap-2 rounded-full bg-gradient-to-r from-[#5B41FF] to-[#8B63F7] px-5 py-2.5 text-sm font-medium text-white hover:opacity-80 transition"
                                >
                                  <Eye size={16} />
                                  Review Session
                                </Link>
                      ...
                      return (
                        <article
                          ...
                          className={`rounded-2xl border p-4 shadow-sm ${
                            isAnswered
                              ? "border-emerald-100 bg-[#F6FFFA]"
                              : "border-slate-100 bg-white"
                          }`}
                        >
                          {/* Header: avatar + ชื่อ + เวลา | status + ปุ่ม */}
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-start gap-3">
                              <div
                                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold ${avatarClasses(question.student_id)}`}
                              >
                              ...
                              <div>
                                ...
                                <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                                  ...
                                  {isBoardQuestion && question.board_title ? (
                                    <span className="rounded-full bg-[#F6F3FF] px-2.5 py-0.5 text-xs font-medium text-[#6F5CC6]">
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                              <span
                                className={`rounded-full px-2.5 py-0.5 text-xs font-medium uppercase ${statusPillClasses(question.status)}`}
                              >
                            ...
                            <div className="mt-2 flex items-center gap-3 flex-wrap">
                              ...
                              <div className="ml-auto flex items-center gap-2">
                                <button
                                  ...
                                  className="rounded-full border border-emerald-200 bg-white px-3 py-1 text-sm font-medium text-emerald-600 hover:bg-emerald-50"
                                >
                                ...
                                <button
                                  ...
                                  className="rounded-full border border-rose-200 bg-white px-3 py-1 text-sm font-medium text-rose-500 hover:bg-rose-50"
                                >
                                  Delete
                                </button>
                              </div>
